class MobilePhone(var myNumber: String){
    ...
    fun queryContact(contactName: String): Contact{
        for (contact in myContacts){
            if (contact.callName == contactName) return contact
        }
        return Contact("", "")
    }

    fun printContacts(){
        myContacts.forEach{e -> println("Contact: ${e.callName} - ${e.phoneNumber}")}
    }
}

data class Contact(var callName: String, var phoneNumber: String){}